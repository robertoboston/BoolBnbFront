...
<template>
	<div id="search-container" class="p-0">
		<div id="searchbar" class="d-flex  align-items-center rounded-5 w-100 bg-white">
			<i class="fa-solid fa-magnifying-glass mx-2"></i>
			<input type="search" class="w-100 b-none" placeholder="inserisci luogo" v-model="search" @keyup="getSugg"
				id="inputSearch">
		</div>
		<!-- dropdown -->
		<ul id="dropdown-sugg" v-if="this.dropdown" class="rounded-3 bg-light list-unstyled w-100">
			<li v-for="item in suggerimenti" :key="item.id" class="p-2" @click="positionClick(item)">
				...
				{{ item.address.country }} {{ item.address.countrySecondarySubdivision }} {{ item.address.countrySubdivision
